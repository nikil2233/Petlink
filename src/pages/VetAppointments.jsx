  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-24 pb-12 px-4 md:px-8">
       ...
       <div className="fixed inset-0 pointer-events-none overflow-hidden -z-10">
          <div className="absolute top-0 right-0 w-[50rem] h-[50rem] bg-indigo-200/20 rounded-full blur-3xl opacity-50"></div>
          <div className="absolute bottom-0 left-0 w-[50rem] h-[50rem] bg-pink-200/20 rounded-full blur-3xl opacity-50"></div>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-6">
            <div>
                <h1 className="text-4xl md:text-5xl font-black text-slate-800 mb-2 tracking-tight">
                    Appointment <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-primary">Dashboard</span>
                </h1>
                <p className="text-lg text-slate-500 font-medium">Manage your clinic's schedule and patient requests.</p>
            </div>

            {/* Stats Cards Row */}
            <div className="flex gap-4">
                <div className="bg-white/80 backdrop-blur-md p-4 rounded-2xl border border-white shadow-sm flex items-center gap-3">
                ...
                <div className="bg-white/80 backdrop-blur-md p-4 rounded-2xl border border-white shadow-sm flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-2xl font-black text-slate-800">{stats.confirmed}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        {/* Filters & Content */}
        <div className="bg-white/60 backdrop-blur-xl rounded-[2.5rem] p-6 md:p-8 shadow-xl border border-white/50 min-h-[600px]">
                ...
                {[
                    ...
                    <button
                        ...
                        className={`
                            ...
                            ${filter === tab.id ? 'bg-white text-primary shadow-md scale-100' : 'text-slate-500 hover:bg-slate-200/50 hover:text-slate-700'}
                        `}
                    >
                        {tab.icon}
                        {tab.label}
            ...
            {loading ? (
                ...
                <div className="text-center py-20">
                    ...
                    <h3 className="text-xl font-bold text-slate-600 mb-2">No Appointments Found</h3>
                    <p className="text-slate-400">There are no {filter} appointments to show right now.</p>
                </div>
            ) : (
                    ...
                    <AnimatePresence mode='popLayout'>
                        {filteredAppointments.map((app, index) => (
                            <motion.div 
                                ...
                                className="group bg-white rounded-2xl p-5 border border-slate-100 shadow-sm hover:shadow-lg hover:border-indigo-100 transition-all cursor-pointer flex flex-col sm:flex-row items-center gap-6 relative overflow-hidden"
                            >
                                {/* Status Indicator Line */}
                                <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${
                                    app.status === 'confirmed' ? 'bg-emerald-500' : 
                                    app.status === 'pending' ? 'bg-amber-400' : 'bg-slate-300'
                                }`}></div>

                                {/* Date Box */}
                                <div className="flex flex-col items-center justify-center bg-slate-50 min-w-[5rem] h-20 rounded-xl border border-slate-200 group-hover:border-indigo-200 group-hover:bg-indigo-50 transition-colors">
                                    ...
                                    <span className="text-2xl font-black text-slate-700 group-hover:text-indigo-600 leading-none">
                                ...
                                <div className="flex-1 w-full text-center sm:text-left">
                                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-1">
                                        <h3 className="text-lg font-bold text-slate-800 flex items-center justify-center sm:justify-start gap-2">
                                            {app.service_type}
                                            {app.status === 'confirmed' && <Check size={16} className="text-emerald-500 bg-emerald-100 p-0.5 rounded-full" />}
                                        </h3>
                                        <span className="text-xs font-semibold px-2 py-0.5 rounded text-slate-500 bg-slate-100 w-fit mx-auto sm:mx-0">
                                            {app.time_slot}
                                        </span>
                                    </div>
                                    ...
                                    {app.status === 'pending' ? (
                                        <div className="flex gap-2">
                                            ...
                                            <button 
                                                ...
                                                className="px-4 py-2 bg-white text-rose-500 border border-slate-200 text-sm font-bold rounded-lg hover:bg-rose-50 hover:border-rose-200 transition-colors"
                                            >
                                                Decline
                                            </button>
                                        </div>
      ...
      {selectedAppointment && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
              <motion.div 
                ...
                className="bg-white rounded-[2rem] max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col overflow-hidden"
              >
                  {/* Header */}
                  <div className="p-8 border-b border-slate-100 flex justify-between items-start bg-gradient-to-b from-slate-50 to-white">
                      <div>
                          <div className="flex items-center gap-2 mb-2">
                              <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide ${
                                  selectedAppointment.status === 'confirmed' ? 'bg-emerald-100 text-emerald-700' : 
                                  selectedAppointment.status === 'pending' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-500'
                              }`}>
                                  {selectedAppointment.status}
                              </span>
                              <span className="text-slate-400 text-xs font-mono">#{selectedAppointment.id.slice(0, 8)}</span>
                          </div>
                          <h2 className="text-3xl font-black text-slate-800 flex items-center gap-2">
                              {selectedAppointment.service_type}
                          </h2>
                      </div>
                      <button onClick={() => setSelectedAppointment(null)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 hover:text-slate-600 transition-colors">
                          <X size={24} />
                      </button>
                  </div>
                  ...
                  <div className="p-8 space-y-8">
                          ...
                          <div className="space-y-4">
                              ...
                              <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100">
                                   <div className="flex items-center gap-4 mb-4">
                                       ...
                                       <div>
                                           <div className="font-bold text-lg text-slate-800">{selectedAppointment.pet_name}</div>
                                           <div className="text-sm text-slate-500">{selectedAppointment.pet_species} • {selectedAppointment.pet_gender}</div>
                                       </div>
                                   </div>
                                   <div className="grid grid-cols-2 gap-2 text-sm">
                                       <div className="bg-white p-2 rounded-lg border border-slate-200">
                                           <span className="text-xs text-slate-400 block">Age</span>
                                           <span className="font-bold text-slate-700">{selectedAppointment.pet_age}</span>
                                       </div>
                                       <div className="bg-white p-2 rounded-lg border border-slate-200">
                                           <span className="text-xs text-slate-400 block">Weight</span>
                                           <span className="font-bold text-slate-700">{selectedAppointment.pet_weight}</span>
                                       </div>
                                   </div>
                              </div>
                          </div>
                          ...
                          <div className="space-y-4">
                              ...
                              <div className="bg-slate-50 p-5 rounded-2xl border border-slate-100 h-full">
                                  <div className="flex items-center gap-4">
                                      ...
                                      <div>
                                          <div className="font-bold text-lg text-slate-800">{selectedAppointment.profiles?.full_name || 'Guest'}</div>
                                          <div className="text-sm text-slate-500">{selectedAppointment.profiles?.email}</div>
                                      </div>
                                  </div>
                              </div>
                          </div>
                      ...
                      <div>
                           ...
                           <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
                                <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                                    <span className="font-semibold text-slate-600">Vaccination Status</span>
                                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${selectedAppointment.vaccinated === 'Vaccinated' ? 'bg-green-100 text-green-700' : 'bg-rose-100 text-rose-700'}`}>
                                        {selectedAppointment.vaccinated}
                                    </span>
                                </div>
                                <div className="flex justify-between items-center border-b border-slate-100 pb-4">
                                    <span className="font-semibold text-slate-600">Overall Health</span>
                                    <span className={`font-bold ${selectedAppointment.is_healthy ? 'text-green-600' : 'text-amber-600'}`}>
                                        {selectedAppointment.is_healthy ? 'Healthy' : 'Has Conditions'}
                                    </span>
                                </div>
                                {(!selectedAppointment.is_healthy || selectedAppointment.on_medication) && (
                                    <div className="bg-amber-50 p-4 rounded-xl text-amber-900 text-sm">
                                        ...
                                        {selectedAppointment.on_medication && (
                                             <p className="mt-2 text-amber-800">Rx: {selectedAppointment.medication_details}</p>
                                        )}
                                    </div>
                                )}
                           </div>
                      ...
                      <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-6 flex items-center justify-between">
                          <div className="flex items-center gap-3">
                              ...
                              <div>
                                  <div className="text-xs text-indigo-400 font-bold uppercase tracking-wider">Date</div>
                                  <div className="font-black text-xl text-indigo-900">{selectedAppointment.appointment_date}</div>
                              </div>
                          </div>
                          <div className="w-px h-10 bg-indigo-200"></div>
                          <div className="text-right">
                              <div className="text-xs text-indigo-400 font-bold uppercase tracking-wider">Time</div>
                              <div className="font-black text-xl text-indigo-900">{selectedAppointment.time_slot}</div>
                          </div>
                      </div>
                      ...
                      {selectedAppointment.status === 'pending' ? (
                          <>
                            ...
                            <button 
                                ...
                                className="px-6 py-3 rounded-xl bg-slate-900 text-white font-bold shadow-lg shadow-slate-300 hover:bg-slate-800 transition-all flex items-center gap-2"
                            >
                                <Check size={18} /> Confirm & Schedule
                            </button>
                          </>
                      ) : (
                          <button onClick={() => setSelectedAppointment(null)} className="px-6 py-3 rounded-xl text-slate-500 font-bold hover:bg-slate-200">Close Details</button>
                      )}
                  </div>
              </motion.div>
          </div>
      ...
      {acceptanceModal && (
          <div className="fixed inset-0 bg-slate-900/40 z-[60] flex items-center justify-center p-4 backdrop-blur-sm">
              <motion.div 
                ...
                exit={{ opacity: 0, scale: 0.95 }}
                className="bg-white rounded-[2rem] max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col"
              >
                  <div className="p-8 border-b border-slate-100">
                      ...
                      <p className="text-slate-500 mt-2">Finalize the schedule and provide care instructions for the owner.</p>
                  </div>
                  ...
                  <div className="p-8 space-y-6">
                      ...
                      <div className="space-y-4">
                        <h4 className="font-bold text-slate-800 border-b border-slate-100 pb-2">Medical Instructions</h4>
                        
                        <div>
                            <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Pre-Surgery / Visit Instructions</label>
                            <textarea 
                                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm text-slate-600 min-h-[80px]"
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Procedure Details</label>
                            <textarea 
                                className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm text-slate-600 min-h-[60px]"
                  ...
                  <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3 sticky bottom-0">
                      ...
                      <button 
                        ...
                        className="px-6 py-3 rounded-xl bg-emerald-500 text-white font-bold shadow-lg shadow-emerald-200 hover:bg-emerald-600 hover:-translate-y-1 transition-all"
                      >
                          Send Confirmation
                      </button>
                  </div>
